namespace System
{
    public static class Math
    {
        ...
        {
            if (value == sbyte.MinValue) throw new OverflowException("Value is too small");
            if (value < 0) return (sbyte)-value;
            return value;
        }

        public static short Abs(short value)
        {
            if (value == short.MinValue) throw new OverflowException("Value is too small");
        ...
        {
            if (value == int.MinValue) throw new OverflowException("Value is too small");
        ...
        {
            if (value == long.MinValue) throw new OverflowException("Value is too small");
            if (value < 0) return -value;
            return value;
        }
        ...
        {
            if ((x > 1.0) || (x < -1.0)) throw new ArgumentOutOfRangeException("Domain error");
            return (pio2 - Asin(x));
        }
        #endregion
        ...
        {
            if (x > 1.0F) throw new ArgumentOutOfRangeException("Domain error");
            double sign = 1F, temp;
            if (x < 0.0F)
            {
                x = -x;
            ...
            temp = Sqrt(1.0F - (x * x));
            if (x > 0.7) temp = pio2 - Atan(temp / x);
        ...
        #region Atan
        public static double Atan(double x) { return ((x > 0F) ? atans(x) : (-atans(-x))); }
        #endregion
        ...
        public static double Ceiling(double a)
        {
            ...
            if (a == Double.NaN || a == Double.NegativeInfinity || a == Double.PositiveInfinity) return a;
            return (a - (int)a > 0) ? (int)(a + 1) : (int)a;
        }
        #endregion
        ...
        public static double Cos(double x)
        {
            ...
            double x2 = x * x;
            if (quadrand == 0 || quadrand == 3) return (c1 + (x2 * (c2 + (x2 * (c3 + (x2 * (c4 + (x2 * (c5 + (x2 * (c6 + (x2 * (c7 + (x2 * (c8 + (x2 * (c9 + (x2 * (c10 + (x2 * c11))))))))))))))))))));
        ...
        #region Log10
        public static double Log10(double x) { return Log(x, 10F); }
        #endregion

        #region Min
        public static byte Min(byte v1, byte v2) { return (v1 < v2) ? v1 : v2; }

        public static sbyte Min(sbyte v1, sbyte v2) { return (v1 < v2) ? v1 : v2; }

        public static ushort Min(ushort v1, ushort v2) { return (v1 < v2) ? v1 : v2; }

        public static short Min(short v1, short v2) { return (v1 < v2) ? v1 : v2; }

        public static uint Min(uint v1, uint v2) { return (v1 < v2) ? v1 : v2; }

        public static int Min(int v1, int v2) { return (v1 < v2) ? v1 : v2; }

        public static ulong Min(ulong v1, ulong v2) { return (v1 < v2) ? v1 : v2; }
        ...
        {
            if (double.IsNaN(v1) || double.IsNaN(v2)) return double.NaN;
            return (v1 < v2) ? v1 : v2;
        }
        #endregion
        ...
        public static sbyte Max(sbyte v1, sbyte v2) { return (v1 > v2) ? v1 : v2; }

        public static ushort Max(ushort v1, ushort v2) { return (v1 > v2) ? v1 : v2; }

        public static short Max(short v1, short v2) { return (v1 > v2) ? v1 : v2; }

        public static uint Max(uint v1, uint v2) { return (v1 > v2) ? v1 : v2; }

        public static int Max(int v1, int v2) { return (v1 > v2) ? v1 : v2; }

        public static ulong Max(ulong v1, ulong v2) { return (v1 > v2) ? v1 : v2; }

        public static long Max(long v1, long v2) { return (v1 > v2) ? v1 : v2; }
        ...
        public static double Pow(double x, double y)
        {
            if (x <= 0.0F)
            {
                double temp = 0F;
                long l;
                if (x == 0.0F && y <= 0.0F) throw new ArgumentException();

                l = (long)Floor(y);
                if (l != y) temp = Exp(y * Log(-x));
                if ((l % 2) == 1) temp = -temp;
        ...
        #region Round
        public static double Round(double d) { return ((Math.Floor(d) % 2 == 0) ? Math.Floor(d) : Math.Ceiling(d)); }
        #endregion
        ...
        public static double Tan(double x)
        {
            ...
            switch (octant)
            {
                case 0: x = x * (4 / PI); break;
                case 1: x = ((PI / 2) - x) * (4 / PI); break;
                case 2: x = (x - (PI / 2)) * (4 / PI); break;
                case 3: x = (PI - x) * (4 / PI); break;
                case 4: x = (x - PI) * (4 / PI); break;
                case 5: x = ((3.5 * PI) - x) * (4 / PI); break;
                case 6: x = (x - (3.5 * PI)) * (4 / PI); break;
                case 7: x = ((2 * PI) - x) * (4 / PI); break;
                default: break;
            }
            ...
            double x2 = x * x;
            if (octant == 0 || octant == 4) return ((x * (c1 + (x2 * (c2 + (x2 * (c3 + (x2 * (c4 + (x2 * c5))))))))) / (c6 + (x2 * (c7 + (x2 * (c8 + (x2 * (c9 + x2))))))));
            if (octant == 1 || octant == 5) return (1 / ((x * (c1 + (x2 * (c2 + (x2 * (c3 + (x2 * (c4 + (x2 * c5))))))))) / (c6 + (x2 * (c7 + (x2 * (c8 + (x2 * (c9 + x2)))))))));
            if (octant == 2 || octant == 6) return (-1 / ((x * (c1 + (x2 * (c2 + (x2 * (c3 + (x2 * (c4 + (x2 * c5))))))))) / (c6 + (x2 * (c7 + (x2 * (c8 + (x2 * (c9 + x2)))))))));
            return -((x * (c1 + (x2 * (c2 + (x2 * (c3 + (x2 * (c4 + (x2 * c5))))))))) / (c6 + (x2 * (c7 + (x2 * (c8 + (x2 * (c9 + x2))))))));
        }
        #endregion

        #region Tanh
        public static double Tanh(double x) { return (expm1(2F * x) / (expm1(2F * x) + 2F)); }
        #endregion

        #region Truncate
        public static double Truncate(double x) { return ((x == 0) ? 0F : ((x > 0F) ? Floor(x) : Ceiling(x))); }
        #endregion
        ...
        private static double atans(double x)
        {
            ...
            if (x > sq2p1) return (pio2 - atanx(1.0F / x));
        ...
        private static double atanx(double x)
        {
            ...
            */
            if ((x > -.01) && (x < .01)) value = (atan_p0 / atan_q0);
            else
            {
                argsq = x * x;
